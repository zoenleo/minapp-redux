...
function wxss(wxssFileList) {
    ...
    return gulp
        ...
        .pipe(
            gulpif(wxssConfig.less && wxssConfig.sourcemap, sourcemaps.init())
        )
        .pipe(gulpif(wxssConfig.less, less({ paths: [srcPath] })))
        .pipe(rename({ extname: '.wxss' }))
        .pipe(
            gulpif(
                wxssConfig.less && wxssConfig.sourcemap,
                sourcemaps.write('./')
            )
        )
...
class BuildTask {
    constructor(id, entry) {
        ...
        this.id = id
        this.entries = Array.isArray(config.entry)
            ? config.entry
            : [config.entry]
    ...
    init() {
        ...
        gulp.task(
            ...
            gulp.series(
                async () => {
                    ...
                    isDemoExists = await _.checkFileExists(
        ...
        gulp.task(`${id}-component-wxml`, done => {
            ...
            if (
                ...
                wxmlFileList.length &&
                !_.compareArray(
                    this.cachedComponentListMap.wxmlFileList,
                    wxmlFileList
                )
            ) {
        ...
        gulp.task(`${id}-component-wxss`, done => {
            ...
            if (
                ...
                wxssFileList.length &&
                !_.compareArray(
                    this.cachedComponentListMap.wxssFileList,
                    wxssFileList
                )
            ) {
        ...
        gulp.task(`${id}-watch-json`, () =>
            gulp.watch(
                this.componentListMap.jsonFileList,
